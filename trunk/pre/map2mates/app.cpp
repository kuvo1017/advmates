...
//改版履歴
//  2013.04.22 新規作成
//  2013.05.29 DRM変換機能追加、MATES本体の交差点形状修正対応
//             緯度経度境界処理追加、これがないとODが外に出ない
...
#define SETUP_KEY_FILETYPE              "MATES_ROADCONV_SETUP"       //設定ファイルキー
#define SETUP_KEY_OSM                   "OSM"
#define SETUP_KEY_DRM                   "DRM"
#define SETUP_KEY_MODIFY                "MODIFY"
#define SETUP_KEY_INPUT_OSM_FILE        "INPUT_OSM_FILE"
#define SETUP_KEY_INPUT_DRM_FILES       "INPUT_DRM_FILES"
...
#define SETUP_KEY_DRM_CHAR_CODE         "DRM_CHAR_CODE"
#define SETUP_KEY_DRM_ROADTYPE_MIN      "DRM_ROADTYPE_MIN"
#define SETUP_KEY_DRM_ROADTYPE_MAX      "DRM_ROADTYPE_MAX"
#define SETUP_KEY_DRM_INTEROP           "DRM_INTEROP"
...
#define SETUP_VAL_CC_EBCDIC             "ebcdic"
#define SETUP_VAL_CC_SJIS               "sjis"
...
App::App()
{
    ...
    _drmCharCode = APP_DRM_CC_EBCDIC;
    _drmRT_Min = 1;
    _drmRT_Max = 10;
    _drmInterOp = true;
    _modMergeLen = 30.0;
...
//          qDebug("read '%s' key '%s'", line.str().c_str(), key.c_str());
            ...
            else if (key == SETUP_KEY_DRM) {
                if (!getSetupBoolData(&key, &line, &_drm))
                    return false;
            }
            ...
            else if (key == SETUP_KEY_INPUT_DRM_FILES) {
                string filename;
                if (!getSetupData(&key, &line, NULL, NULL, &filename))
                    return false;
                _inputDRM_Files.push_back(filename);
            }
            ...
            else if (key == SETUP_KEY_DRM_CHAR_CODE) {
                string charCode;
                if (!getSetupData(&key, &line, NULL, NULL, &charCode))
                    return false;
                if      (charCode == SETUP_VAL_CC_EBCDIC)
                    _drmCharCode = APP_DRM_CC_EBCDIC;
                else if (charCode == SETUP_VAL_CC_SJIS)
                    _drmCharCode = APP_DRM_CC_SJIS;
                else {
            ...
            else if (key == SETUP_KEY_DRM_ROADTYPE_MIN) {
                if (!getSetupData(&key, &line, &_drmRT_Min))
                    return false;
            }
            else if (key == SETUP_KEY_DRM_ROADTYPE_MAX) {
                if (!getSetupData(&key, &line, &_drmRT_Max))
                    return false;
            }
            else if (key == SETUP_KEY_DRM_INTEROP) {
                if (!getSetupBoolData(&key, &line, &_drmInterOp))
                    return false;
            }
    ...
    if (!_osm && !_drm && !_modify) {
        cerr << "no setup file function "
             << SETUP_KEY_OSM << " " << SETUP_KEY_DRM << " " << SETUP_KEY_MODIFY << endl;
        return false;
    }
    ...
    if (_inputDRM_Files.empty() && _drm) {
        cerr << "no setup file key " << SETUP_KEY_INPUT_DRM_FILES << endl;
        return false;
    }
    if (_inputMatesDir == "" && !_osm && !_drm) {
