...
{
public:
    Intersection(const std::string& id, const std::string& type);
    ...
    /**
     * @pram result_inter 交差点内の交錯レーン
     * @pram result_section 交錯レーンに接続する上流単路内のレーン
     */
    ...
    /// 指定したレーンへの進入を許可するか
    /**
     * ついでに徐行（制限付き許可）を扱うため、bool型からint型に変更した
     * @return 0 許可なし
     * @return 1 許可あり（無制限）
     * @return 2 許可あり（徐行） 
     * @return 3 許可あり（一旦停止）
     */
    int hasPermission(Lane* lane, Vehicle* vehicle) const;
