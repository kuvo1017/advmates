...
public:
    ...
    /// 隣のレーンでの位置を得る
    /**
     * @param result_isLeft 左にあれば+1，右にあれば-1
     **/
    double lengthOnSideLane(const Lane* thisLane,
                            const Lane* sideLane,
                            double length,
                            int* result_isLeft) const;

    /// @p thatLane は @p thisLane の左側にあるか
    /**
     * @return +1 左側にある
     * @return -1 右側にある
     * @return  0 判別できない
     */
    int isLeftLane(const Lane* thisLane,
                   const Lane* thatLane,
                   double length) const;
...
public:
    ...
    /// @in1 から @in2 へ向かう方向に通過するのに予想される所要時間を返す
