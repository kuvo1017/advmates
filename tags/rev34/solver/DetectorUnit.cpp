			       ...
			       ulint interval)
{
    ...
    for (int i=0; i<static_cast<signed int>(_unitSvd.laneIds.size()); i++)
    {
...
void DetectorUnit::monitorLanes(){
    _unitPvd.clear();
    if (TimeManager::time()%_interval==TimeManager::unit())
    {
        clearStatVehicleData(_unitSvd);
    }

    for (int i=0; i<static_cast<signed int>(_detectors.size()); i++)
    {
        // 個々の感知器の処理
        ...
        if (_detectors[i]->getPassedVehicleData(&result_pvd))
        {
            _unitPvd.insert(_unitPvd.end(),
                            result_pvd.begin(),
                            result_pvd.end());
            for (int j=0; j<static_cast<signed int>(result_pvd.size()); j++)
            {
