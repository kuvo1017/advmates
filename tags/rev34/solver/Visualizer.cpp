...
int Visualizer::_idleEventIsOn     = false;
int Visualizer::_withCapturing     = false;
int Visualizer::_outputTimeline    = false;
int Visualizer::_outputInstrumentD = false;
int Visualizer::_outputInstrumentS = false;
int Visualizer::_outputGenCounter  = false;
int Visualizer::_outputTripInfo    = false;
...
int Visualizer::_isSectionId       = false;
int Visualizer::_isLaneId          = false;
int Visualizer::_isLanesInter      = true;
int Visualizer::_isLanesSection    = true;
int Visualizer::_isSimpleMap       = false;
int Visualizer::_isSubsectionShape = true;
int Visualizer::_isSubnetwork      = false;
int Visualizer::_isSubsectionId    = false;
int Visualizer::_surfaceMode       = 0;
int Visualizer::_isSignals         = true;
int Visualizer::_isRoadsideUnits   = false;
int Visualizer::_connectorIdMode   = 0;

int Visualizer::_isVehicleId       = false;
...
void Visualizer::drawRoadsideUnits()
{
    ...
    for (int i=0; i<static_cast<signed int>(detectorUnits->size()); i++)
    {
...
void Visualizer::drawVehicles()
{
    ...
    for (unsigned int i=0; i<allVehicles->size(); i++)
    {
        if ((*allVehicles)[i]->isAwayFromOriginNode())
        {
            ...
            {
                vehicleDrawer->drawSimple(*(*allVehicles)[i],
                                          _isVehicleId,
                                          size);
            }
        }
...
{
    // シミュレータの出力フラグを更新
    _sim->setOutputState(_outputTimeline,
                         _outputInstrumentD,
                         _outputInstrumentS,
                         _outputGenCounter,
                         _outputTripInfo);
    _sim->setMonitorState(_outputInstrumentD,
                          _outputInstrumentS);
    ...
    if (_withCapturing)
    {
        ...
        gvm.getVariable("RESULT_IMG_DIRECTORY", &fileName);
...
void Visualizer::generateVehicleManual()
{
    ...
    // 入力された情報から車両を生成
    _sim->generateVehicleManual(startId, goalId, stopPoints,
