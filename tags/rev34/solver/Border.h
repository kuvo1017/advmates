...
public:
    ...
    /// コネクタとともに線分を作成
    /**
     * @p ptBegin, @p ptEnd は境界線の両端の点。デストラクタで消去されない。<br>
    ...
    /** 保持している LineSegment 型のオブジェクトを消去 */
    virtual ~Border();
...
protected:
    ...
    /// コネクタを作成する関数。コンストラクタから呼ばれる。
    /** すべてのコネクタは等間隔に配置される */
    void _createConnectors();

    /** d0 : d1 の内分点にコネクタを作成する */
