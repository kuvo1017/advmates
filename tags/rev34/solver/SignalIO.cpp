...
SignalIO& SignalIO::instance()
{
    ...
    if (!instance._init)
    {
        ...
        // 以下のファイルは現状では使ってない
        gvm.getVariable("RESULT_OUTPUT_DIRECTORY", &resultDir);
        gvm.getVariable("RESULT_SIGNAL_STATIC_FILE", &resultFile);
        instance._staticOutFileName = resultDir+resultFile;

        // 以前の結果を消去する
        if (resultFile != "")
        {
            instance._staticOut.open(instance._staticOutFileName.c_str(),
                                     ios::trunc);
            if (instance._staticOut.good())
            {
                instance._staticOut.close();
            }
        }
    }
				      ...
				      const int sideNum) const
{
    ...
    result
        = (gvm.getVariable("SIGNAL_CONTROL_DIRECTORY", &defaultPath));
    assert(result);
    result
        = (gvm.getVariable("ASPECT_FILE_EXTENSION", &aspectExtension));
    assert(result);
    ...
    else
    {
        ...
        // デフォルトのテーブルを開く
        result = (gvm.getVariable("SIGNAL_ASPECT_FILE_DEFAULT_PREFIX",
                                  &defaultAspectPrefix));
        assert(result);
        ostringstream oss;
...
SignalControlDataSet SignalIO::signalControlDataSet(const string& id) const
{
    ...
    assert(result);
    result
        = (gvm.getVariable("CONTROL_FILE_EXTENSION", &extension));
    assert(result);
    result
        = (gvm.getVariable("SIGNAL_CONTROL_FILE_DEFAULT",
                           &defaultFilename));
    assert(result);
...
(const ulint time, const map<string, Signal* ,less<string> >* signals)
{
    // ファイル名を決定する
    vector<string> paths;
    ...
    string resultDir;
    GVManager& gvm = GVManager::instance();
    gvm.getVariable("RESULT_TIMELINE_DIRECTORY", &resultDir);
    ...
    if (gvm.getNumeric("OUTPUT_COMMENT_IN_FILE")!=0)
    {
        fzprintf(_dynamicOut, "%s\n", _timePrefix.c_str());
    }
    
    map<string, Signal*, less<string> >::const_iterator itl;
    for (itl=signals->begin(); itl!=signals->end(); itl++)
    {
        // 信号現示の出力
        _writeSignalDynamicData( (*itl).second );
    }
