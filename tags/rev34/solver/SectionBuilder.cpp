        ...
        if (_numIn[i]==_numOut[(i+1)%2])
        {
            ...
            for (int j=0; j<_numIn[i]; j++)
            {
                ...
                _lanes[id]->setSpeedLimit
                    (gvm.getNumeric("SPEED_LIMIT_SECTION"));
            }
        }
        else if (_numIn[i]+1==_numOut[(i+1)%2])
        {
            ...
            //最内のレーンの作成
            {
                ...
                double rightTurnLaneLength
                    = gvm.getNumeric("RIGHT_TURN_LANE_LENGTH");
                ...
                if (length<=rightTurnLaneLength)
                {
                    ...
                    _lanes[id] = new Lane(id, pointBegin, pointEnd1);
                    _lanes[id]->setSpeedLimit
                        (gvm.getNumeric("SPEED_LIMIT_SECTION"));
                    ...
                    _lanes[id] = new Lane(id, pointBegin, pointEnd2);
                    _lanes[id]->setSpeedLimit
                        (gvm.getNumeric("SPEED_LIMIT_SECTION"));
                }
                else
