...
bool LocalLaneRouter::_search(const Section* section,
			      const Lane* lane,
			      const double length,
			      vector<Lane*>* result_lanes0,
			      vector<Lane*>* result_lanes1)
{
					...
					const double length) const
{
    Lane* sideLane = NULL;
    double sideLength = 0;
    section->getLeftSideLane(lane, length, &sideLane, &sideLength);
    if (sideLane != NULL)
    {
					 ...
					 const double length) const
{
    Lane* sideLane = NULL;
    double sideLength = 0;
    section->getRightSideLane(lane, length, &sideLane, &sideLength);
    if (sideLane != NULL)
    {
        (*result_Lanes).push_back(sideLane);
        (*result_Length).push_back(sideLength);
 ...
 *
 * @param result_lanes 結果のレーン列 [inout]
						   ...
						   bool includeShiftLane) const
{
    ...
    {
        // 次のレーンもsectionの中にいる
        int loopMax = section->numNextLane(startLane);
        ...
        for(int i=0; i < loopMax && !result; i++)
        {
            ...
            Lane* nextLane = const_cast<Lane*>(section->nextLane(startLane, order[i]));
    ...
    else
    {
        ...
        {
            result_lanes->push_back(startLane);
            Lane* nextLane = const_cast<Lane*>(section->nextLane(startLane,
                                                                 order[i]));
						       ...
						       bool includeShiftLane) const
{
    ...
    {
        // 次のレーンも交差点の中にいる
        int loopMax = intersection->numNextLane(startLane);
        // 探索順序をランダムにすることでランダム選択可能にする．
        vector<int> order = randomOrder(loopMax);
        // 次のレーンを順に探索する
        for (int i=0; i<loopMax && !result; i++ )
        {
            ...
            Lane* nextLane = const_cast<Lane*>(intersection->nextLane(startLane,
                                                                      order[i]));
            result = _getExitLanesInFrontIntersection(result_lanes,
                                                      intersection,
                                                      nextLane,
    ...
    else
    {
        ...
        if (intersection->direction(startLane->endConnector())==objectiveDirection)
        {
            ...
            { 
                result = true;
                int num = intersection->numNextLane(startLane);
                (*result_lanes).push_back(intersection->nextLane(startLane,
                                                                 Random::uniform(num)));
            }
            else
            {
                ...
                for (int i=0; i<loopMax && !result; i++)
                {
                    Lane* nextLane = intersection->nextLane(startLane, order[i]);
						  ...
						  Intersection* frontIntersection) const
{
    ...
    {
        startLane = section->mostStraightNextLane(startLane);
        assert(startLane);
        exitLanes.push_back(startLane);
    }
    startLane = section->mostStraightNextLane(startLane);
    ...
    {
        startLane = frontIntersection->mostStraightNextLane(startLane);
        assert(startLane);
        if (frontIntersection->isMainLane(startLane))
        {
            _localRoute->setTurning(frontIntersection->relativeDirection(startLane));
    ...
    int num = frontIntersection->numNextLane(startLane);
    startLane = frontIntersection->nextLane(startLane,
                                            Random::uniform(num));
    assert(startLane);
    exitLanes.push_back(startLane);
				       ...
				       const double thisLength) const
{
    ...
    if ((inter->signal()->hasPermission(dir))==0)
    {
        return false;
    }
...
//======================================================================
bool LocalLaneRouter::_isUncrowdedRoute(const vector<Lane*>* route0,
					const vector<Lane*>* route1) const
{
    return false;
}
//======================================================================
