...
//======================================================================
void Router::setTrip(Intersection* start, Intersection* goal,
		     RoadMap* roadMap)
{
    _start = start;
    _goal = goal;
    _map = roadMap;
    _od.setValue(_start->id(), _goal->id());
}

//======================================================================
void Router::setTrip(Intersection* start, Intersection* goal,
		     OD* od, RoadMap* roadMap)
{
    _start = start;
    _goal = goal;
    _map = roadMap;
			   ...
			   vector<Route*>& routes)
{
    ...
    if (canSolve && isEnd(goal))
    {
        ...
        vector<Route*>::iterator it = routes.end();
        it--;
		    ...
		    Route*& route)
{
    ...
    // 結果をコピーする前に最初のstartをrouteに格納しておく
    ...
    route = new Route();
    route->push(past);
    route->setLastPassedIntersection(past);
...
bool Router::isGoal(NodeAStar* node, const Intersection* goal)
{
    ...
    if (node == NULL)
    {
        cerr << "router find error start " << _start->id()
             << " goal " << _goal->id() << endl;
        exit(1);
    }
...
double Router::funcG(NodeAStar* node)
{
    ...
    else if(node->parent()->parent() == NULL)
    {
        ...
        x[0] = tmpVec.size();
        //x[1]の算出。
        x[1] = link1->time(na1->top(),node->top());
        //x[2]の導出
        x[2] = 0.0;
        //x[3]の導出
        x[3] = 0.0;
