                                 ...
                                 double crosswalkWidth)
{
    ...
    _sidewalkWidth  = sidewalkWidth;  // 現状使ってない
...
bool Intersection::createBaseStructure(bool readFile)
{
    ...
    if (readFile)
    {
        ...
        string defaultPath;
        gvm.getVariable("INTERSECTION_ATTRIBUTE_DIRECTORY",&defaultPath);
...
bool Intersection::createDefaultStreetVertices()
{
    ...
    for (int i=0; i<static_cast<signed int>(_next.size()); i++)
    {
        ...
        mv.setZ(0);
        double maxMidVec;
        GVManager::instance().getVariable("INTERSECTION_SIZE_LIMIT",
                                          &maxMidVec);
...
bool Intersection::createVertices()
{
    ...
    for (unsigned int i = 0; i < _next.size(); i++)
    {
            ...
            !(_vertices[outerIndex2] == _streetVertices[outerIndex2]) )
        {
            ...
            // 交差の補正、頂点の中点を使う
            if (outerLine01.checkIntersectionkZ0(outerLine23))
            {
    ...
    for (unsigned int i = 0; i < _next.size(); i++)
    {
            ...
            || reviseCorner[(i + _next.size() - 1) % _next.size()])
        {
             ...
             * 車道境界点に合わせて外部頂点を直す
             */
            if (!streetLine0.checkIntersectionkZ0
            ...
            if (!streetLine1.checkIntersectionkZ0
...
bool ODNode::createDefaultStreetVertices()
{
    ...
    tmpX = _center.x()
        -nv.x()*((_numIn[0]+_numOut[0])*0.5*_incSections[0]->laneWidth()
                 +_incSections[0]->roadsideWidth())+dv.x()*tmpWidth;
    tmpY = _center.y()
        -nv.y()*((_numIn[0]+_numOut[0])*0.5*_incSections[0]->laneWidth()
...
bool Intersection::createDefaultBorders()
{
    ...
    for (int i=0; i<static_cast<int>(_next.size()); i++)
    {
        Border* border = new Border(_borderPoints[2 * i],
                                    _borderPoints[2 * i + 1],
                                    _numIn[i],_numOut[i],
                                    _incSections[i]->roadsideWidth());
        assert(border);
...
bool ODNode::createDefaultBorders()
{
    ...
    Border* border0 = new Border(_borderPoints[0], _borderPoints[1],
                                 _numIn[0],_numOut[0],
    ...
    Border* border1 = new Border(_borderPoints[2], _borderPoints[3],
                                 _numOut[0], _numIn[0],
                                 _incSections[0]->roadsideWidth());
    assert(border1);
    addBorder(border1, 2);
    ...
    for (unsigned int i=0; i<_vertices.size(); i++)
    {
        if (i%2==0)
        {
            if (crosswalkWidth(edge2dir(i))!=0)
            {
                ...
                {
                    ptSubI->setWalkerTypes(walkerTypes, TRAFFIC_SIGNAL_RED);
                }
    ...
    for (unsigned int i=0; i<_borders.size(); i++)
    {
        if (_numIn[i]>0)
        {
            ...
            for (unsigned int j=0; j<_borders.size(); j++)
            {
                ...
                // 相対方向が左折の場合
                // 境界iのコネクタ0(最外の流入点)から
                // 境界jのコネクタ_numIn[j]+_numOut[j]-1(最外の流出点)へ
                if ((*_rdTable)(i,j) == RD_LEFT)
                {
                    ...
                    idIntBegin = i*100;
                    idIntEnd = j*100+_numIn[j]+_numOut[j]-1;
                    idInt = idIntBegin*10000+idIntEnd;
 ...
 const Connector* pointEnd, bool setLaneBegEnd)
{
    ...
    _lanes[id] = new Lane(id, pointBegin, pointEnd);
    _lanes[id]->setSpeedLimit(gvm.getNumeric("SPEED_LIMIT_INTERSECTION"));
    if (setLaneBegEnd)
    {
