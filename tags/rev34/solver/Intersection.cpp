...
//======================================================================
Intersection::Intersection(const string& id, const string& type) 
    : LaneBundle(id)
{
...
//======================================================================
vector<Lane*> Intersection::nextLanes(const Lane* lane) const
{
    vector<Lane*> result_lanes = lanesFromConnector(lane->endConnector());
    if (result_lanes.empty())
    {
        Section* nextSection
            = _incSections[direction(lane->endConnector())];
        ...
        //assert(nextSection);
        result_lanes
            = nextSection->lanesFromConnector(lane->endConnector());
    }
    assert(!result_lanes.empty());
    return result_lanes;
}

//======================================================================
vector<Lane*> Intersection::previousLanes(const Lane* lane) const
{
    vector<Lane*> result_lanes = lanesToConnector(lane->beginConnector());
    if (result_lanes.empty())
    {
        Section* prevSection
            = _incSections[direction(lane->beginConnector())];
        result_lanes
            = prevSection->lanesToConnector(lane->beginConnector());
    }
    assert(!result_lanes.empty());
    return result_lanes;
}
    ...
    for (int i=0; i<static_cast<signed int>(main_lanes.size()); i++)
    {
        ...
        else
        {
            ...
            if (v0*v1<=0) {
                ...
                {
                    vector<Lane*> prevLanes = previousLanes(main_lanes[i]);
        ...
        for (itl=_lanes.begin(); itl!=_lanes.end(); itl++)
        {
                ...
                ((*itl).second->lineSegment(), &tmpPoint))
            {
                         ...
                         (*itl).second) == result_inter->end())
                {
                    ...
                    // 上流にさかのぼる
                    vector<Lane*> previousLanes
                        = this->previousLanes((*itl).second);
                    while (!previousLanes.empty())
                    {
                        if (isMyLane(previousLanes[0]))
                        {
                            // 指定したレーン集合に含まれる，
                            // あるいはすでに登録済み
                            if (find(lanes->begin(),
                                     lanes->end(),
                                     previousLanes[0])!=lanes->end()
                            ...
                            {
                                // result_interに新たに登録
                                result_inter->push_back(previousLanes[0]);
                                // さらに上流レーンに展開し，
                                // previousLanesの最後尾に追加する
                                vector<Lane*> morePreviousLanes
                                    = this->previousLanes(previousLanes[0]);
                                previousLanes.insert
                                    (previousLanes.end(),
                        ...
                        else
                        {
                                ...
                                ==result_section->end())
                            {
                                ...
                                result_section->push_back(previousLanes[0]);
                                // これ以上展開しない
                                /*
                                 * レーンが短ければ展開するのも有り
                                 */
                        ...
                        // 先頭の要素を削除
                        previousLanes.erase(previousLanes.begin());
...
//======================================================================
int Intersection::hasPermission(Lane* lane, Vehicle* vehicle) const
{
    assert(isMainLane(lane));

    //レーンの始点の境界番号は識別番号の1 000 000の位
    int beginDir = (atoi(lane->id().c_str())%10000000)/1000000;
    RelativeDirection rd = relativeDirection(lane);

    SignalMainState mainColor;
    SignalSubState subColor;

    mainColor = _signal->mainColor(beginDir);
    subColor = _signal->subColor(beginDir);
    
    if (mainColor==SignalColor::yellow())
    {
        mainColor = _signal->prevMainColor(beginDir);
        subColor = _signal->prevSubColor(beginDir);
    }
