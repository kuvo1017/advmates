...
bool RoadMapBuilder::buildIntersections()
{
    ...
    gvm.getVariable("MAP_POSITION_FILE", &fMapPosition);
    gvm.getVariable("MAP_NETWORK_FILE", &fNetwork);
                ...
                if (type.length()==2)
                {
                    ptInter
                        ...
                        (AmuConverter::formatId
                         (id, NUM_FIGURE_FOR_INTERSECTION), type);
                }
                else
                {
                    ptInter
...
bool RoadMapBuilder::setIntersectionStructInfo(bool readFile)
{
    if (!_isIntersectionStructureCreated)
    {
        ...
        gvm.getVariable("INTERSECTION_STRUCT_FILE", &fStruct);
        ...
        sidewalkWidth  = gvm.getNumeric("DEFAULT_SIDEWALK_WIDTH");
        crosswalkWidth = gvm.getNumeric("DEFAULT_CROSSWALK_WIDTH");
        ...
        while (iti != _currentRoadMap->intersections()->end())
        {
                 ...
                 i++)
            {
                ...
                if ((*iti).second->id() < (*iti).second->next(i)->id())
                {
                    ...
                    {
                        Section* ptSection = new Section(id, begin, end);
...
bool RoadMapBuilder::setSectionStructInfo(bool readFile)
{
    if (!_isSectionStructureCreated)
    {
        ...
        gvm.getVariable("SECTION_STRUCT_FILE", &fStruct);
        int autoSidewalkLane =
            (int)gvm.getNumeric("AUTO_SIDEWALK_SECTION_LANE");
        ...
        // 単路の構造情報に既定値を設定する、設定済みなら内部で無視する
        laneWidth     = gvm.getNumeric("DEFAULT_LANE_WIDTH");
        roadsideWidth = gvm.getNumeric("DEFAULT_ROADSIDE_WIDTH");
        sidewalkWidthRight = sidewalkWidthLeft
            = gvm.getNumeric("DEFAULT_SIDEWALK_WIDTH");
...
bool RoadMapBuilder::setSpeedLimit()
{
    ...
    gvm.getVariable("SPEED_LIMIT_FILE", &fSpeedLimit);
    ...
    while (ifs.good())
    {
        ...
        if (!str.empty())
        {
            ...
            {
                // 値をセットする
                const map<string, Lane*, less<string> >* lanes
                    = section->lanes();
