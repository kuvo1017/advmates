...
void DetectorIO::getReadyOutputFiles(vector<DetectorUnit*>* units)
{
    ...
    for (unsigned int i=0; i<units->size(); i++) 
    {
        ...
        gvm.getVariable("RESULT_INSTRUMENT_DIRECTORY", &outputDir);
        gvm.getVariable("RESULT_DETECTORD_PREFIX", &prefixD);
        gvm.getVariable("RESULT_DETECTORS_PREFIX", &prefixS);
        ...
        if (!outputDir.empty())
        {
            ...
            // 計算を何回も行う場合など
            ofstream outDetailFile(fDetail.c_str(), ios::out);
            ...
            // 同名のファイルが既に存在する場合は内容が破棄されるので注意．
            // 計算を何回も行う場合など
            ofstream outStatFile(fStat.c_str(), ios::out);
...
void DetectorIO::print()
{
    ...
    cout << "*** Detector Information ***" << endl;
